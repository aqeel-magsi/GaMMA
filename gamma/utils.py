...
def convert_picks_csv(picks, stations, config):
    '''df = picks.sort_values("phase_time")
    df.phase_time = pd.to_datetime(df.phase_time)
    df.index = df.phase_time
    dfUsed = df[config["starttime"]:config["endtime"]]
    if type(dfUsed["timestamp"].iloc[0]) is str:
        dfUsed.loc[:, "timestamp"] = dfUsed["timestamp"].apply(lambda x: datetime.fromisoformat(x))
    t = (
        dfUsed["timestamp"]
    ...
    if config["use_amplitude"]:
        a = dfUsed["amp"].apply(lambda x: np.log10(x * 1e2)).to_numpy()  ##cm/s
        data = np.stack([t, a]).T
    else:
        data = t[:, np.newaxis]
    meta = stations.merge(dfUsed["id"], how="right", on="id", validate="one_to_many")
    locs = meta[config["dims"]].to_numpy()
    phase_type = dfUsed["type"].apply(lambda x: x.lower()).to_numpy()
    phase_weight = dfUsed["prob"].to_numpy()[:, np.newaxis]
    pick_station_id = dfUsed.apply(lambda x: x.id + "_" + x.type, axis=1).to_numpy()
...
def association(df, stations, config, event_idx0=0, method="BGMM", **kwargs):
    #>>>>>>>>>>>>>>. Modified by Kai <<<<<<<<<<<<<<<<<
    # to cut the time range
    start = config["starttime"]
    end = config["endtime"]
    after_start = df[pd.to_datetime(df.phase_time)>=start]
    picks = after_start[pd.to_datetime(after_start.phase_time)<end]
    #>>>>>>>>>>>>>>>>>>> 2023 Nov 13 <<<<<<<<<<<<<<<<<<<<<<<<
    #print(picks)
    ...
    if ("use_dbscan" in config) and config["use_dbscan"]:
        ...
        #print(data[:, 0:1])
        #print(data)
        #print(locs[:, :2]/ np.average(vel['p']))
        #print(np.hstack([data[:, 0:1], locs[:, :2] / np.average(vel["p"])]))
        labels = db.labels_
        #print(labels)
        unique_labels = set(labels)
        unique_labels = unique_labels.difference([-1])

        #>>>>>>>>>>>>>>>>>>>>>>>>. try to plot clustring <<<<<<<<<<<<<<<<<<<<<
        ...
        for i in range(len(labels)):
            ...
            if int(labels[i]) == -1:
                colors.append("black")
        #print(colors)
        ax.scatter3D(data[:,:1],locs[:, :1]/np.average(vel["p"]), locs[:, 1:2]/np.average(vel["p"]), c=colors)
...
def associate(
    ...
    #print(pick_idx[labels == k], k, labels)
